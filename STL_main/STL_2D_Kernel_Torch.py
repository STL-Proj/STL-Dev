...
class WaveletOperator2Dkernel_torch:
    ...
    def _gaussian_kernel_5x5(self, device, dtype, pbc: bool = True):
        ...
        if (
            ...
            # ---- all arithmetic on CPU in float64; cast and match device at the end ----
            ...
            f = torch.fft.fftfreq(Nd, dtype=torch.float64)
            fx, fy = torch.meshgrid(f, f, indexing="ij")
            nu = torch.sqrt(fx**2 + fy**2) * 2.0  # [0, √2]; Nyquist = 1

            # --- isotropic raised-cosine frequency response ---
            H = torch.zeros(Nd, Nd, dtype=torch.float64)
            H[nu <= nu1] = 1.0
            mask = (nu > nu1) & (nu < nu2)
            H[mask] = 0.5 * (1.0 + torch.cos(torch.pi * (nu[mask] - nu1) / (nu2 - nu1)))
            ...
            hann = torch.hann_window(K, periodic=False, dtype=torch.float64)
